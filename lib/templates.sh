...
#############################################
# Template Functions
# Generates user-customizable deploy.sh and config
#############################################

# Create deployment configuration file
# This stores repository and branch info for release.sh
create_deploy_config() {
    local username=$1
    local repository=$2
    local branch=$3
    local home_dir="/home/$username"
    local config_file="$home_dir/.deploy-config"
...
# Auto-generated by Cipi - DO NOT EDIT MANUALLY
# Use 'cipi app edit' to modify these settings
...
create_deploy_script() {
    ...
    create_deploy_config "$username" "$repository" "$branch"
...
# Update deploy config (used when editing app settings)
update_deploy_config() {
    ...
    case "$field" in
        repository)
            local clone_url=$(git_url_to_ssh "$value")
            sed -i "s|^REPOSITORY=.*|REPOSITORY=\"$clone_url\"|" "$config_file"
            ;;
        branch)
            sed -i "s|^BRANCH=.*|BRANCH=\"$value\"|" "$config_file"
            ;;
        *)
            echo -e "${RED}Error: Unknown config field: $field${NC}"
            return 1
            ;;
    esac
}
